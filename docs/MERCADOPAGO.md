...
| Archivo | Qué hace |
|---|---|
| `src/components/MercadoPagoWallet.tsx` | Monta el Wallet Brick. `initMercadoPago(VITE_MP_PUBLIC_KEY)` + `<Wallet initialization={{ preferenceId }} />`. Si falta la Public Key o el `preferenceId`, muestra un placeholder de "pendiente de configuración" (no rompe). |
